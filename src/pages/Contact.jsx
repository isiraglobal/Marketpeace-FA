    ...
    <div className="w-full min-h-screen pt-20 sm:pt-24 md:pt-32 px-4 sm:px-6 md:px-12 flex flex-col items-center pb-12 sm:pb-16 md:pb-24">
      ...
      >
        ...
        <h1 className="text-2xl sm:text-4xl md:text-7xl font-black tracking-tighter mb-8 md:mb-10 text-white leading-[0.95] drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)] uppercase italic">
          Let's <span className="not-italic text-[#1B7FDC]">Talk</span>
        </h1>
        ...
        <div className="lg:col-span-7 flex flex-col gap-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <ContactMethod 
              icon={<Mail />} 
              title="Email Us" 
              value="foreignaffairsllc2017@gmail.com" 
              link="mailto:foreignaffairsllc2017@gmail.com"
            />
            <ContactMethod 
              icon={<Instagram />} 
              title="Follow Us" 
          ...
          <div className="liquid-glass p-8 md:p-12 relative overflow-hidden group">
            ...
            <h3 className="text-2xl md:text-3xl font-black text-white mb-4 flex items-center gap-4 uppercase italic title">
              <MapPin className="text-[#0DB8D3]" /> FOR VENDORS
            </h3>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 inline-block w-full sm:w-auto">
              <p className="text-[#0DB8D3] font-black tracking-widest text-[10px] uppercase mb-2 amount">Direct Curation Email</p>
              <p className="text-white text-base md:text-xl font-black break-all title">foreignaffairsllc2017@gmail.com</p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              ...
              <input id="contact-name"
                ...
                required 
                className="w-full bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4 text-white outline-none focus:border-[#0DB8D3] text-sm backdrop-blur-md"
                value={formData.name}
            ...
            <div>
              ...
              <select id="contact-subject"
                className="w-full bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4 text-white/70 outline-none focus:border-[#0DB8D3] text-sm backdrop-blur-md appearance-none"
                value={formData.subject}
                onChange={e => setFormData({...formData, subject: e.target.value})}
              >
                <option value="General Inquiry" className="bg-[#193546]">General Inquiry</option>
                <option value="Vendor Question" className="bg-[#193546]">Vendor Question</option>
                <option value="Venue Partnership" className="bg-[#193546]">Venue Partnership</option>
                <option value="Promoter Inquiry" className="bg-[#193546]">Promoter Inquiry</option>
              </select>
            </div>
            <div>
              ...
              <textarea id="contact-message"
                ...
                required 
                className="w-full bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4 text-white outline-none focus:border-[#0DB8D3] h-32 text-sm backdrop-blur-md"
            ...
            <button 
              ...
              className="w-full mt-4 py-3 sm:py-4 md:py-5 bg-white text-[#193546] font-black rounded-xl sm:rounded-2xl tracking-[0.1em] md:tracking-[0.2em] text-[12px] sm:text-xs uppercase hover:bg-white/90 transition-all shadow-xl flex items-center justify-center gap-2 sm:gap-3 btn-glow"
            >
...
function ContactMethod({ icon, title, value, link }) {
  return (
    <a href={link} target="_blank" rel="noreferrer" className="flex flex-col gap-4 p-8 liquid-glass hover:bg-white/10 transition-all group">
      <div className="w-12 h-12 rounded-2xl bg-[#0DB8D3]/10 flex items-center justify-center text-[#0DB8D3] group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <div>
